...
question_file=word2vec/question-words.txt
...
echo "## Three epochs then two epochs"
...
echo "## Skip-gram"
bin/multivec-mono --train ${corpus}.tok.en --save ${model_dir}/model.sg.en.bin --iter 8 --save-vectors ${model_dir}/vectors.sg.en.txt --skip-gram --threads 16
bin/compute-accuracy ${model_dir}/vectors.sg.en.txt ${voc_size} < ${question_file}
