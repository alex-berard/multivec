...
float MonolingualModel::similaritySentenceSyntax(const string& seq1, const string& seq2, const string& tags1, const string& tags2, const string& idf1, const string& idf2, float alpha, int policy) const {
    ...
    auto idf_weight1 = split(idf1);
    auto idf_weight2 = split(idf2);
    ...
    for (size_t i = 0; i < words1.size() && i < pos_tags1.size(); ++i) {
        try {
            vec1 += wordVec(words1[i], policy) * pow(syntax_weights.at(pos_tags1[i]), 1 - alpha) * pow(std::stof(idf_weight1[i]), alpha);
        }
        catch (runtime_error) {}
    }
    ...
    for (size_t i = 0; i < words2.size() && i < pos_tags2.size(); ++i) {
        try {
            vec2 += wordVec(words2[i], policy) * pow(syntax_weights.at(pos_tags2[i]), 1 - alpha) * pow(std::stof(idf_weight2[i]), alpha);
        }
        catch (runtime_error) {}
    }
...
float BilingualModel::similaritySentenceSyntax(const string& src_seq, const string& trg_seq, const string& src_tags, const string& trg_tags, const string& src_idf, const string& trg_idf, float alpha, int policy) const {    
    ...
    auto src_idf_weight = split(src_idf);
    auto trg_idf_weight = split(trg_idf);
    ...
    for (size_t i = 0; i < src_words.size() && i < src_pos_tags.size(); ++i) {
        try {
            src_vec += src_model.wordVec(src_words[i], policy) * pow(syntax_weights.at(src_pos_tags[i]), 1 - alpha) * pow(std::stof(src_idf_weight[i]), alpha);
        }
        catch (runtime_error) {}
    }
    for (size_t i = 0; i < trg_words.size() && i < trg_pos_tags.size(); ++i) {
        try {
            trg_vec += trg_model.wordVec(trg_words[i], policy) * pow(syntax_weights.at(trg_pos_tags[i]), 1 - alpha) * pow(std::stof(trg_idf_weight[i]), alpha);
        }
        catch (runtime_error) {}
    }
